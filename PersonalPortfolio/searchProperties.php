...
<head>
	...
	<link async href="css/bootstrap-theme.min.css" rel="stylesheet">
	...
	<script>
	function getPropertyPic(propertyPicId){
		$.ajax({
			method: "GET",
			url: "webservices/getPropertyPic.php",
			data: {
				propertyPicId:propertyPicId
			}
		}).done(function( msg ) {
			console.log('successful ajax to getPropertyPic.php');
			console.log(msg);
			//$('#img-propertyId-174').attr('src','data:image/jpeg;base64,'+msg);
			...
			if ($_GET['PropertyType'] == 1 || $_GET['PropertyType'] == 0){
				...
				foreach( $results as $row ){
					...
					<div class="panel-heading">
						<h3 class="panel-title"><?php echo ''.$row['address'].'';?></h3>
						<?php
					...
					<table class="table">
						<tr>
							<td>
								<div id='Picture'>
									<!--<img id = 'img-propertyId-<?php echo $row['propertyId']; ?>' style="max-width: 250px; max-height: 250px" src="data:image/jpeg;base64,<?php  /*echo base64_encode( $row["image"] );*/ ?>"/>-->
									<img id = 'img-propertyId-<?php echo $row['propertyId']; ?>' style="max-width: 250px; max-height: 250px" src="http://52.26.240.26/webservices/getPropertyPic.php?propertyPicId=23"/>
								</div>
							</td>
						</tr>
					<?php
