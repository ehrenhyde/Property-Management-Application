...
function ctrl_input_field($errors, $inputType,$isRequired,$name, $labelText,$className,$originalValue=null,$tabindex=null){
	...
	if ('hidden' != $inputType){
		...
		$strInputField = "<input type =\"$inputType\" id = \"$name\" name = \"$name\" value = \"$value\" class = \"$className\" tabindex=\"$tabindex\"/>";
...
function ctrl_select($errors,$name,$values,$className,$useDisplayAsValue = false,$tabindex=null){
	echo "<select id = \"$name\" name = \"$name\" tabindex=\"$tabindex\">";
...
function ctrl_input_radio($errors,$name,$values,$labels,$className,$defaultValue,$tabindex){
	...
	for($iV = 0;$iV<count($values);$iV++){
		...
		echo "<input id=\"r-{values[$iV]}\" type=\"radio\" name=\"$name\" value=\"$values[$iV]\" $checked tabindex='$tabindex'>";
...
function ctrl_submit($value = 'Submit',$id = 'btnSubmit',$tabindex = null){
	echo "<input id = '$id' type = 'submit' name = 'submit' value = '$value' tabindex='$tabindex' />";
	echo '<br>';
}
