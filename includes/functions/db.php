...
function db_addTenant_temp($confirmCode,$email,$password,$firstName,$lastName,$DOB,$isMale){
	try{
		$pdo = db_connect();
		$stmt = $pdo->prepare(		
					'INSERT INTO tenant_temp(confirmCode,email,password,firstName,lastName,DOB,isMale) VALUES (:confirmCode,:email,:password,:firstName,:lastName,:DOB,:isMale);'
				);
...
function db_search_w_house($address,$minRooms,$maxRooms, $minPrice, $maxPrice){
	$pdo = db_connect();
	try{
		
		$sSearchAddress = "%".$address."%";
		$stmt = $pdo->prepare(
						'select * from v_search_w_house '.
						'where address like :searchVal '.
						'AND numRooms >= COALESCE(:minRooms,0) '.
						'AND numRooms <= COALESCE(:maxRooms,9999999999) '.
